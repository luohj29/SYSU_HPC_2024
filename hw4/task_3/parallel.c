...
int parallel_for(float * A,float*B,float*C,int start, int end, int increment, void *(*functor)(void*), void *arg , int num_threads){
    ...
    // struct for_index *index_set = (struct for_index *)malloc(sizeof(struct for_index)* num_threads);
    for (int i = 0; i < num_threads; i++) {
        end_i += avg;
        if(i==num_threads-1){
            end_i = end; //compute the left
        }
        struct for_index index_i= {start_i, end_i, 0, A,B,C};
        printf("%d %d\n", index_i.start,index_i.end);
        pthread_create(&pthread_set[i], NULL, functor, (void*) &index_i); //开启并行
        start_i = end_i;
    }

    // struct for_index index_i;
    // // struct for_index *index_set = (struct for_index *)malloc(sizeof(struct for_index)* num_threads);
    // for (int i = 0; i < num_threads; i++) {
    //     end_i += avg;
    //     if(i==num_threads-1){
    //         end_i = end; //compute the left
    //     }
    //     index_i.start = start_i;
    //     index_i.end = end_i;
    //     index_i.increment = increment;
    //     index_i.A = A;
    //     index_i.B = B;
    //     index_i.C = C;
    //     printf("%d %d\n", index_i.start,index_i.end);
